    ...
    <main class="dashboard">
      ...
      <section class="hero-card">
        ...
        <div class="hero-status">
          ...
          <div id="main-status" class="status-value">STOPPED</div>
        </div>
      </section>
      ...
      <section class="activity-grid">
        ...
        <div class="panel">
          <div class="panel-header">
            <div>
              <div class="panel-title">Proxy Activity</div>
              <div class="panel-subtitle">Current session statistics</div>
            </div>
          </div>
        ...
        <div class="panel">
          <div class="panel-header">
            <div>
              ...
              <div class="panel-subtitle">Latest proxy events</div>
            </div>
  ...
  >
    ...
    >
      <path
        ...
        1.19-3.1-.12-.29-.52-1.47.11-3.06 0 0 .97-.31 3.18 1.18a11.06 11.06 0 0 1
...
function setStatus(running: boolean, pid?: number) {
  const mainStatus = document.querySelector("#main-status");
  const headerStatus = document.querySelector("#header-status");

  if (!mainStatus || !headerStatus) return;
  ...
  if (running) {
    ...
    if (pidElement && pid !== undefined) {
      pidElement.textContent = String(pid);
    }
  } else {
...
async function refreshStatus() {
  try {
    ...
    if (running) {
      const processes = await invoke<number | null>("proxy_pid");
      setStatus(true, processes ?? undefined);
    } else {
      setStatus(false);
    }
  } catch (error) {
...
startButton?.addEventListener("click", async () => {
  try {
    ...
    alert(`Failed to start Snowflake proxy:\n${error}`);
  } finally {
...
stopButton?.addEventListener("click", async () => {
  try {
    ...
    alert(`Failed to stop Snowflake proxy:\n${error}`);
  } finally {
